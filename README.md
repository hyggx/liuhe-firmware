...
**六合**（英文：liuhe）是针对泉盛 UV-K6 V1 的第三方开源固件，基于
...
| **信道操作流程优化** | 存储信道/编辑名称确认后直接保存，不再显示无用的 WAIT 步骤；编辑区居中对齐，显示光标下划线，单次 MENU 键进入编辑 | 菜单 → 存储信道 / 编辑名称 |
| **静噪稳定性修复** | 灵敏静噪模式下开/关阈值方向计算错误，导致弱信号区间出现噪音断续；已修复并经实机验证 | — |
| **FM 调频优化** | 换台前自动静音，消除切台爆音 | FM 模式 ↑/↓ |
| **AM 接收质量提升** | 目标信号电平 -89→-95 dBm（满调制余量）；初始增益修复（消除冷启动弱信号盲区）；RSSI 滤波优化（减少增益频繁跳变） | — |
| **S 表精度修复** | 消除 RSSI 取整系统性误差（-0.5 dBm）；修正 AM 增益补偿参考点（-7 dBm 误差）；加入显示平滑滤波，弱信号时 S 表不再剧烈跳动 | — |
| **S 表 IARU R1 校准** | S9 默认值修正为 -73 dBm，符合 IARU 第一区技术建议 | — |
